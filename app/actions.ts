  ...
  "rag.ingest_from_github": {
    ...
    parameters: {
      ...
      properties: {
        ...
        max_repo_size_mb: { type: "number" },
        convert_to_md: { type: "boolean" },
        push_to_github: { type: "boolean" },
  ...
  "document.convert": {
    ...
    parameters: {
      ...
      properties: {
        ...
        chunking_strategy: { type: "string", enum: ["by_title", "by_size", "none"] },
      },
...
export async function chatWithAgent(messages: Message[]): Promise<string> {
  ...
  if (data.choices?.[0]?.message?.tool_calls) {
    ...
    for (const toolCall of toolCalls) {
      ...
      try {
        ...
        results.push(`[${toolName}] ${JSON.stringify(output)}`);
      } catch (error) {
        results.push(`[${toolName}] Error: ${error instanceof Error ? error.message : "Unknown error"}`);
      }
    }

    const followUpResponse = await fetch(`${MINIMAX_API_BASE}/v1/text/chatcompletion_v2`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${MINIMAX_API_KEY}`,
...
- 深圳(shenzhen)支持 multi-project 操作，哈工大本部/威海(harbin/weihai)不支持`,
    ...
    return followUpData.choices?.[0]?.message?.content || JSON.stringify(data);
  }
